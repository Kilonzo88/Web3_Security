...
## 📖 Description
Occurs when **multiple contracts share the same state**, typically through a shared storage contract or token. The attacker exploits inconsistent state updates across contract boundaries. This is extremely dangerous in modular DeFi protocols.

## 🎯 Key Characteristics
- State shared across multiple contracts
- Harder to detect than single-contract issues
- Common in DeFi protocols with vault + token patterns
- OpenZeppelin's ReentrancyGuard alone won't protect
